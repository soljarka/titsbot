...
func SendTits(update tgbotapi.Update, bot *tgbotapi.BotAPI, titsnum int, titslink string) {
	if update.Message.From.UserName == "ruelephant" || (update.Message.From.UserName != "ruelephant" && WroteUsers.CheckInterval(update.Message.From)) || update.Message.Chat.IsPrivate() {
